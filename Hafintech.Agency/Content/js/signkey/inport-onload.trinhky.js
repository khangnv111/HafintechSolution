                ...
                case CMD.SIGN_TXT:
                    ...
                    if (wscpp.state == STATE.SUCCESS) {
                        if (wscpp.vData.length >= 2) {
                            ...
                            //document.getElementById('txtDataSigned').value = signData;

                            var keyData = KString.utf8Decode(wscpp.vData[1]);
                            //document.getElementById('txtPublicKey').value = keyData;

                            var edi = KString.utf8Decode(wscpp.vData[2]);
                            //document.getElementById('txtMessage').value = edi;

                            var dclId = 0;
                            var dclType = 0; 
                            if (app) {
                                dclId = app.data.dclId;
                                ...
                                if (this.readyState == 4 && this.status == 200) {
                                    ...
                                    if (ress.ResponseCode > 0) {
                                        bootbox.alert("Thành công!", function () {
											if (app && app.jobCode == "IDA") {
												if (status == 5) {
													window.location.href = Config.Url_Root + "Declaration/HightValueDeInsert?IdDec=" + data.Data.dclId + "&ishight=2&tab=5";
												}
                                                else if (app.statusTK == 6) {
                                                    window.location.href = Config.Url_Root + "Declaration/HightValueDeInsert?IdDec=" + ress.Data.dclId + "&ishight=2&tab=6";
                                                }
                                                else if (app.statusTK == 7) {
                                                    window.location.href = Config.Url_Root + "Declaration/HightValueDeInsert?IdDec=" + ress.Data.dclId + "&ishight=2&tab=6";
                                                }
                                                else {
                                                    window.location.href = Config.Url_Root + "Declaration/HightValueDeInsert?IdDec=" + ress.Data.dclId + "&ishight=2&tab=3";
                                                }
                                            }
                                            else {
                                                location.reload();
