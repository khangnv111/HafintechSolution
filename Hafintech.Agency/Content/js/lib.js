var Lib = new function () {
    ...
    //chi cuc hai quan
    this.getCustomsOffice = async function (cstOfficeCd, cstOfficeNm) {
        try {
            var token = $('#hdfToken').val();
            if(cstOfficeCd == undefined || cstOfficeCd == null) cstOfficeCd = "";
            if(cstOfficeNm == undefined || cstOfficeNm == null) cstOfficeNm = "";

            const response = await axios.get(Config.API_Login + "tax/GetCustomsOffice?cstOfficeCd=" + cstOfficeCd + "&cstOfficeNm=" + cstOfficeNm,
                {
                    headers: { "Authorization": "Bearer " + token }
                });
            var res = response.data;
