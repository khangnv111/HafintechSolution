					...
					if (wscpp.state == STATE.SUCCESS) {
						if (wscpp.vData.length >= 2) {
							...
							if (app) {
								...
								amendDclNo = app.data.amendDclNo;
							}
							var signedData = {
								"data": edi,
								"signature": signData,
								...
								if (this.readyState == 4 && this.status == 200) { 
									...
									if (ress.ResponseCode > 0) {
										bootbox.alert("Thành công!", function () {
											if (dclType == 731) {
												window.location.href = Config.Url_Root + "AMA/InsertUpdateAMA?decId=" + ress.Data.id + "&tab=1&ishight=6";
											}
											else if (dclType == 733) {
												window.location.href = Config.Url_Root + "ama/InsertUpdateAMA?decId=" + ress.Data.id + "&tab=3&ishight=6";
											}
											else {
												location.reload();
											}
