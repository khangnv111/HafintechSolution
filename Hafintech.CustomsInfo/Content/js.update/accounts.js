...
$('#imgPasspostImgPath').html("<a href='" + Config.API_Login + "Data\\upload\\" + dt.PassportImgPath + "' data-lightbox='roadtrip'><img src='" + Config.API_Login + "Data\\upload\\" + dt.PassportImgPath + "' style='height:50px;' /> </a>");
    ...
    accounts.SearchUser = function () {
        ...
        $.ajax({
            ...
            data: {
                accountID: $('#txtAccountID').val() == "" ? "0" : $('#txtAccountID').val(),
                email: $('#txtEmail').val(),
            ...
            success: function (value) {
                if (value) {
                    ...
                    var str = "<table id='listContainer' class='table table-bordered data-table table-striped with-check' role='grid' aria-describedby='example1_info'><thead><tr><th class='text-left'>AccountID</th><th class='text-left'>Username</th><th class='text-left'>FullName</th><th class='text-left'>Identity</th><th class='text-left'>Mobile</th><th class='text-left'>Address</th><th class='text-left'>CreatedDate</th><th class='text-left'>UpdatedDate</th></tr></thead><tbody>";
                    if (value.length > 0)
                    {
                        $.each(value, function (index, data) {
                            if (data.Username != null) {
                                str += ("<tr><td><a href='" + Config.Url_Root +"home/accountinfo?uid=" + data.AccountID+"'>" + (data.AccountID != null ? data.AccountID : "") + "</a></td><td>" + (data.Username != null ? data.Username : "") + "</td><td>" + (data.FullName != null ? data.FullName : "") + "</td><td>" + (data.Identity != null ? data.Identity : "") + "</td><td>" + (data.Mobile != null ? data.Mobile : "") + "</td><td>" + (data.Address != null ? data.Address : "") + "</td><td>" + (data.CreatedDate != null ? accounts.formDateTime(data.CreatedDate) : "") + "</td><td>" + (data.UpdatedDate != null ? accounts.formDateTime(data.UpdatedDate) : "") + "</td></tr>");
                            }
    ...
    accounts.SearchUser_More = function () {
        accounts.ShowLoading();
        ...
        $.ajax({
            type: 'GET',
            url: Config.API_Login + "account/Search",//"service/FindUser",
            data: { 
                accountId: accountId,
                identity: identity,
                mobile: mobile,
            ...
            success: function (value) {
                ...
                if (value) {
                    ...
                    str += "<th class='text-left'>AccountID</th>";
                    ...
                    if (value.ResponseCode > 0 && value.Data != null && value.Data != undefined && value.Data.length > 0) {
